...
def EuclideanBarycenter(images):
    ...
    n = len(images[0])  # len of the flattened image
    N = range(n)
    avg = [0 for _ in N]

    for row in images:  # For each flattened image
        for i in N:
            avg[i] += row[i]

    k = len(images)  # total number of images
    for i in N:
        avg[i] = avg[i] / k
        ...
        for j in range(N):
            for v in range(N):
                for w in range(N):
                    if ground == "L1":
                        G.add_edge(
                            ID(i, j), n + ID(v, w), weight=abs(i - v) + abs(j - w)
                        )
                    if ground == "L2":
                        G.add_edge(
                            ID(i, j),
                            n + ID(v, w),
                            weight=sqrt(pow(i - v, 2) + pow(j - w, 2)),
                        )
                    if ground == "Linf":
                        G.add_edge(
    ...
    for K in [50]:
        for gd in GD:
            for n in NUM:
                ...
                logger.info(
                    "Test: {} Images {} GD: {} Readtime: {:f} Runtime: {:f} Obj: {:f}".format(
                        n, K, gd, read_time, time.time() - start, obj
                    )
                )
                fh.flush()
                # Dump barycenter
    ...
    for i in range(n):
        ...
        for k in range(K):
            m.addConstr(
                ...
                - quicksum(X[k, j][k, i] for j, _ in Bs)
                == images[k][i] - Z[i]
            )
...
def TestPaperL1():
    ...
    for K in [50]:  # [50, 100, 200, 400, 800, 1600, 3200]:
        for n in NUM:
            ...
            logger.info(
                "Test: {} Images {} Readtime: {:f} Runtime: {:f} Obj: {:f}".format(
                    n, K, read_time, time.time() - start, obj
                )
            )
            fh.flush()
            # Dump barycenter
    ...
    for v in G.nodes():
        ...
        for k in range(K):
            m.addConstr(
                ...
                - quicksum(X[i, j, k] for i, j in Bs)
                == images[k][v] - Y[v]
            )
        ...
        for j in range(N):
            for v, w in Cs:
                if i + v >= 0 and i + v < N and j + w >= 0 and j + w < N:
                    G.add_edge(
                        ID(i, j), ID(i + v, j + w), weight=sqrt(pow(v, 2) + pow(w, 2))
                    )
    ...
    for K in [50, 100, 200, 400, 800, 1600, 3200]:
        for n in NUM:
            for L in Ls:
                ...
                logger.info(
                    "L: {} Test: {} Images {} Readtime: {:f} Runtime: {:f} Obj: {:f}".format(
                        L, n, K, read_time, time.time() - start, obj
                    )
                )
                fh.flush()
                # Dump barycenter
